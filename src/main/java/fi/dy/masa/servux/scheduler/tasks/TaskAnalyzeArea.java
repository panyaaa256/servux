...
public class TaskAnalyzeArea extends TaskChunkWalkerBase
{
	 ...
	 * <p>
	 * Takes {@link ItemInstance} because that is the one type both a real stack in a slot
	 * and a template inside a container component satisfy, which is what makes the recursion
	 * through nested containers a single method.
	 */
	private void addItem(ItemInstance item)
	{
		...
		Identifier id = BuiltInRegistries.ITEM.getKey(item.typeHolder().value());

		if (id != null)
		{
			this.result.addContainerItem(id, item.count());
		}
	}
}
