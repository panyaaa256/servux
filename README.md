...
- 不一致は `Missing` (足りない)、`Extra` (余計)、`Wrong Block` (ブロック違い)、`Wrong State` (状態違い)、`Wrong Contents` (中身違い) に分類されます。
...
`<分類>` は `missing` / `extra` / `wrong_block` / `wrong_state` / `wrong_nbt` のいずれかです。
...
- **エンティティ:** 検証で比較するのはブロックとコンテナの中身です。エンティティの比較はまだしません。
